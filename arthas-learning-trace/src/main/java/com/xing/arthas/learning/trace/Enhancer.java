...
import com.xing.arthas.learning.trace.source.Constants;
import com.xing.arthas.learning.trace.source.FieldUtils;
import com.xing.arthas.learning.trace.source.FileUtils;
...
//            }
            ...
            try {
                ...
                System.err.println(String.format("print spy failed. classname=%s;loader=%s;", className, inClassLoader));
                t.printStackTrace();
                throw t;
            }
            ...
            System.err.println(String.format("transform loader[%s]:class[%s] failed.", inClassLoader, className));
            t.printStackTrace();
        }
    ...
    private static void dumpClassIfNecessary(String className, byte[] data) {
        ...
        if (!classPath.mkdirs()
                && !classPath.exists()) {
            System.err.println(String.format("create dump classpath:%s failed.", classPath));
            return;
        }
        ...
        try {
            ...
            System.err.println(String.format("dump class:%s to file %s failed.", className, dumpClassFile));
            e.printStackTrace();
        }
...
//    }
    ...
    public static synchronized void enhance(
            ...
            final String classNameMatcher,
            final String methodNameMatcher) throws UnmodifiableClassException {
        ...
        // 构建增强器
        final Enhancer enhancer = new Enhancer(adviceId, isTracing, skipJDKTrace, enhanceClassSet, methodNameMatcher);
        try {
            ...
            if (false) {
                ...
                if (classArray.length > 0) {
                    ...
                    System.out.println("Success to batch transform classes: " + Arrays.toString(classArray));
                }
            } else {
                // for each 增强
                for (Class<?> clazz : enhanceClassSet) {
                    try {
                        ...
                        System.out.println("Success to transform class: " + clazz);
                    } catch (Throwable t) {
                        System.err.println(String.format("retransform %s failed.", clazz));
     ...
     *
     * @param inst             inst
     * @param className 类名匹配
     * @return 增强影响范围
     * @throws UnmodifiableClassException
     */
    public static synchronized void reset(
            final Instrumentation inst,
            final String className) throws UnmodifiableClassException {

        final Set<Class<?>> enhanceClassSet = new HashSet<Class<?>>();

        for (Class<?> classInCache : classBytesCache.keySet()) {
            if (className.equals(classInCache.getName())) {
        ...
        try {
            ...
            System.out.println("Success to reset classes: " + enhanceClassSet);
        } finally {
